...
public abstract class BaseAsyncTaskLoader<T> extends AsyncTaskLoader<T> {
  ...
  public void deliverResult(final T data) {
    if (DEBUG) { Log.d(TAG, "deliverResult(" + data + ")"); }
    if (isReset()) { return; }
    result = data;
    busy = false;
  ...
  protected void onStartLoading() {
    if (DEBUG) { Log.d(TAG, "onStartLoading()"); }
  ...
  protected void onStopLoading() {
    if (DEBUG) { Log.d(TAG, "onStopLoading()"); }
    cancelLoad();
    busy = false;
  }
  ...
  public void onCanceled(final T data) {
    if (DEBUG) { Log.d(TAG, "onCanceled(" + data + ")"); }
  ...
  protected void onForceLoad() {
    if (DEBUG) { Log.d(TAG, "onForceLoad()"); }
    super.onForceLoad();
    busy = true;
  }
