...
;;; Now assumes first thing on classpath is what should be used. (GSK)
(define appDir
  (let* ((cp ($ "java.class.path"))
         ...
         (subdir (cond ((= i -1) cp)
                       ...
                       (else (error (string-append
                                     "appDir: can't create File object "
                                     "from classpath "
                                     "\"" cp "\"\n"))))))
    (.getCanonicalFile (.getParentFile (.getCanonicalFile (File. subdir))))))
  ...
  ;; Version and build date displayed when JScheme starts.
	...
	(display
	 (let* ((d (.format (DateFormat.getDateTimeInstance DateFormat.SHORT$
							    DateFormat.SHORT$)
			    (Date.))))
	 {JScheme 7.3 ([d]) http://jscheme.sourceforge.net\n}
	 ) s))))
