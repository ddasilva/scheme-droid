...
public class Import {
  ...
  **/
  ...
  public static Class forName(String name) {
    ...
    if (loader == null) 
      try { klass = Class.forName(name); }
      catch (ClassNotFoundException e) { /* fall back */}

    if ( null != klass ) return klass;

    if ( loader != null ) {
	try { return Class.forName(name,false,loader);} 
            // fix by Michael Thome to handle Java 6 regression in ClassLoader.loadClass syntax, 16Jun08
      catch (ClassNotFoundException e) { /* fall back */ }
      // KRA 28JUN00: Renu found this!    
      catch (NoClassDefFoundError e) { /* fall back */ } 
    }

    if ( null != klass ) return klass;

    try { klass = Class.forName(name); }
    catch (ClassNotFoundException e)  { /* last attempt */ }
